...
class GroqProvider extends BaseProvider
{
    ...
    {
        $this->apiKey = config('agent.providers.groq.api_key') ?: env('GROQ_API_KEY');
        $this->baseUrl = config('agent.providers.groq.base_url', 'https://api.groq.com/openai/v1');
    }
    ...
    public function analyze(string $text, ModelConfig $config): AnalysisResult
    {
        ...
        try {
            ...
            Log::error('Groq analysis failed', ['error' => $e->getMessage()]);
    ...
    {
        throw new \RuntimeException('Groq does not support embeddings');
    }
