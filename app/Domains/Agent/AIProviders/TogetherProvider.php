...
class TogetherProvider extends BaseProvider
{
    ...
    {
        $this->apiKey = config('agent.providers.together.api_key') ?: env('TOGETHER_API_KEY');
        $this->baseUrl = config('agent.providers.together.base_url', 'https://api.together.xyz/v1');
    }
    ...
    public function chat(array $messages, ModelConfig $config): AIResponse
    {
        ...
        if (!$response->successful()) {
            ...
            throw new \RuntimeException("Together API error: {$response->body()}");
        }
    ...
    public function analyze(string $text, ModelConfig $config): AnalysisResult
    {
        ...
        try {
            ...
            Log::error('Together analysis failed', ['error' => $e->getMessage()]);
    ...
    {
        throw new \RuntimeException('Together does not support embeddings');
    }

    public function models(): array
    {
        return AIProvider::Together->availableModels();
    }
